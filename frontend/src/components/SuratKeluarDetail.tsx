...
export function SuratKeluarDetail({ id, onClose }: SuratKeluarDetailProps) {
  ...
  const handleExportPdfSistem = async () => {
    try {
      ...
      a.href = url;
      a.download = `cetak-surat-keluar-${id}.pdf`;
  ...
  const statusConfig: Record<string, { label: string; badgeClass: string }> = {
    ...
    SENT: { label: 'Terkirim', badgeClass: 'bg-emerald-50 text-emerald-700 border-emerald-200' },
    ARCHIVED: { label: 'Diarsipkan', badgeClass: 'bg-blue-50 text-blue-700 border-blue-200' },
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">Surat Keluar / Detail</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 rounded-xl border border-gray-100 bg-gray-50/50">
        ...
        <div>
          ...
          <span className="text-sm font-semibold text-gray-800">Surat Keluar</span>
        </div>
        <div>
          <span className="text-xs text-gray-400 font-medium block">Penerima</span>
          <span className="text-sm font-semibold text-gray-800">{dataSurat.penerima || '-'}</span>
        </div>
        <div>
          <span className="text-xs text-gray-400 font-medium block">Pengirim</span>
          <span className="text-sm font-semibold text-gray-800">Internal / Logistik</span>
        </div>
      </div>
